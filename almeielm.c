 ...
 * @date July 27 2021
 *
 * This file handles the implementation of helper functions that generate the population dataset.
 * In addition, delete by record, and read from file.
 */
...
int generateSeniors(int household_size) {
    // get a random number for the number of seniors (aged above 50) in the household
    return rand() % household_size + HOUSEHOLD_SIZE_OFFSET;
}

/**
 * Provide the number of people living in the household that are not senior citizens.
 *
 * @param household_size
 * @param seniors
...
/**
 * Generate a random number of adults (not seniors [over 50]) that have chronic diseases.
 *
 * @param adults
 * @return number of adults who have chronic diseases
 */
int generateAdultsWithChronicDiseases(int adults) {
    // get a random number for adults with chronic diseases in the household. if adults is 0, return 0
    return (adults > 0) ? rand() % adults + HOUSEHOLD_SIZE_OFFSET : 0;
}

/**
 * Generate a random number of people tested for covid based on the number of people living in the household.
 *
 * @param household_size
...
/**
 * Generate a random number of adults tested positive for COVID-19 that is between the number of adults that exists
 * and the number of people tested for covid.
 *
 * @param adults
 * @param tested
...
household generateHousehold() {
    // create new household item -- use the aforementioned methods to create a new household item
    household item;
...
/**
 * Print the header that showcases what data is in the columns -- this should be called before the loop that goes into
 * printing each household item.
 */
void printHouseholdItemHeader() {
    // header formatting
    printf("--------------------------------------------------------------------------------------------------------------------\n");
    printf("%5s\t%7s\t%16s\t%20s\t%20s\t%15s\t%15s\n", "S.no", "Size", "Total Tested", "Total Tested Positive", "Race", "Region", "Town");
    printf("--------------------------------------------------------------------------------------------------------------------\n");
}
...
void printHouseholdItem(int ctr, household item) {
    // print all the household data points in a specified format matching the aforementioned header
    printf("%5d\t%7d\t%16d\t%20d\t%20s\t%15s\t%15s\n", ctr, item.size, item.tested, item.adultsTestedPositive, displayRace(item.race), displayRegion(item.region), displayTown(item.region, item.town));
}
...
void printHouseholdItemFooter() {
    // disclaimer i guess
    printf("---------------------- This program analyzes and stores information related to COVID-19 cases ----------------------\n");
    printf("----------------------------- and its impact on various races, regions and towns of GTA ----------------------------\n");
}
    ...
    do {
        ...
        switch(regionChoice) {
            ...
            case 2:
                ...
                printLinkedListByRegion(head, regionChoice); // print the linked list filtered by the region specified
            ...
            case 9:
                ...
                break;
            default:
                printf("Invalid data, Enter an integer 0 through 2 or enter 9 to go back to the main menu. Try again.\n\n");
                break;
        }
    } while (regionChoice != 9); // only when the regionChoice entered by the user is 9 does the program exit this loop
}
...
void displayHouseholdsByTown(Node *head) {
    ...
    } while(regionChoice != 9 || townChoice != 9); // do - while until the region or town entered is 9
}
...
void displayHouseholdsByRace(Node *head) {
    ...
    do {
        ...
        printf("Enter an integer for race: African American (0) Asian (1) Caucasian (2) Indigenous (3) Other (4)\n");
        scanf("%d", &raceChoice);
        ...
        switch(raceChoice) {
            ...
            case 4:
                ...
                break;
            default:
                printf("Invalid entry. Please choose between 0 through 4 or 9 to exit. Try again.\n");
                break;
        }
...
/**
 * Display the households by region with a specified minimum of adults tested positive.
 *
    ...
    do {
        ...
        switch(regionChoice) {
            ...
            case 2:
                ...
                scanf("%d", &minTestedPositive); // get the minimum adults positive from the user
                printLinkedListByRegionWithMinTestedPositive(head, regionChoice, minTestedPositive); // print the linked list filtered by the region and minimum tested positive
...
int getRegionInput() {
    ...
    } while(regionChoice < 0 || regionChoice > 2); // keep looping if invalid expectation
    return regionChoice; // return region entered
}

/**
 * Get the town input from the user depending on the region previously chosen by the user
 *
 * @param region
...
int getTownInput(int region) {
    ...
    do {
        ...
        if (townChoice < 0 || townChoice > 1)
            printf("Invalid input. Try again.\n");
    } while (townChoice < 0 || townChoice > 1); // loop until the town entered is valid
...
int getRaceInput() {
    ...
    do {
        ...
        printf("Race: ");
        scanf("%d", &raceInput);
    } while (raceInput < 0 || raceInput > 4); // keep looping until the correct (0 - 4) race is entered
    return raceInput;
}

/**
 * Get household information such as the household size, the number of seniors living in the household and the number of
 * adults (under 50) with chronic diseases living in the household.
 *
 * @param householdSize
 * @param householdSeniors
 * @param householdAdultsWithChronicDiseases
 */
void getHouseholdInformationInput(int *householdSize, int *householdSeniors, int *householdAdultsWithChronicDiseases) {
    int validInput = 0;
    int size, seniors, adultsWithChronicDiseases;
    do {
        // get the household size, seniors, and adults with chronic diseases from the user
        printf("Please enter\n1 - Total number of people in the household,\n2 - People over the age of 50,\n3 - People under the age of 50 with chronic diseases separated by space/tab and make sure all of these are valid integers\n");
        scanf("%d %d %d", &size, &seniors, &adultsWithChronicDiseases);
        // check to see if the household size is less than the seniors entered, or if the household size if less than the adults with chronic diseases entered
        // or check to see if the seniors + adults with chronic diseases are greater than the household size entered
        if ((seniors + adultsWithChronicDiseases) > size || size < seniors || size < adultsWithChronicDiseases)
            printf("Total people in the household can't be less than the people over 50 or people under 50 with chronic diseases or even their sum. Kindly re-enter the data.\n");
        ...
        // everything is fine
            ...
            validInput = 1;
    } while(!validInput); // keep looping until determine a valid input for all 3 (household size, seniors and adults with chronic diseases)

    // will always be valid input if program reaches here -- pointer so multiple data can be changed
    *householdSize = size;
...
/**
 * Get the number of people tested and the number of people tested positive in the household from the user
 *
 * @param householdTested
 * @param householdTestedPositive
 */
int getHouseholdCovidInformationInput(int householdSize, int *householdTested, int *householdTestedPositive) {
    int validInput = 0, tested, testedPositive;
    char continueInput;
    do {
        // get the tested and tested positive in the household from the user
        printf("Enter total number of people in the household who were tested for COVID-19 and those who were tested positive separated by space or tab.\n");
        scanf("%d %d", &tested, &testedPositive);
        // check to see if the tested are greater than the household size or if the tested positive is greater than the tested
        if (tested > householdSize || testedPositive > tested) {
            printf("Total people in the household cannot be less than the total members tested for COVID-19\nSimilarly total tested can't be less than the total tested positive\nEnter 'Y' if you want to re-enter/correct household members or\nEnter 'F' if you want to re-enter only COVID-19 test figures\n");
            // flush input stream
            while (getchar() != '\n');
            ...
            if (continueInput == 'Y') {
                ...
                continue; // continue the program -- will ensure that tested and positive tested match the conditions set
            }
        // everything is fine
        } else {
            validInput = 1;
...
Node* addRecord(Node *head) {
    ...
    do {
        ...
        // get the household covid information -- if response if 0, household information needs to be updated, if anything else (99.9% sure 1), will continue
        response = getHouseholdCovidInformationInput(size, &tested, &testedPositive);
    } while (response == 0);

    // create new household record
    household item = createHousehold(region, town, race, size, seniors, adultsWithChronicDiseases, tested, testedPositive);
    // add the record
    addNode(&head, item);
...
Node* deleteByRegionTownRace(Node *head) {
    ...
    // send the request to delete the household record based on the region, town and race
