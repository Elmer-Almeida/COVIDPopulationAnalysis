 ...
 *
 * The main objective of the program will be to generate population data, and store it in a linked list.
 * The ability to store data into a file and read from a file will also be available.
 */
...
int main() {
    ...
    Node *head = makeNode(itemHead); // add the first created household to the head of the linked list

    // add randomized data to the linked list
    for (int household_ctr = 1; household_ctr < HOUSEHOLD_LIMIT; household_ctr++) {
        household item = generateHousehold(); // generate a new household with randomized data
        addNode(&head, item); // add the new randomized data household to the linked list
    }
    ...
    do {
        ...
        switch(userChoice) {
            ...
            case 3:
                ...
                break;
            case 4:
                // display households of a region with a given minimum number of people tested positive for COVID-19
                displayByRegionMinTestedPositive(head);
                break;
            case 5:
                // display the regions town-wise ranking of number of people tested positive for COVID-19
