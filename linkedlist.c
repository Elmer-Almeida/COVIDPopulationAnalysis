 ...
 * @date July 27 2021
 *
 * This file handles the implementation of linked list associated methods defined in linkedlist.h
 */
...
void addNode(NodePtr *head, household item) {
    NodePtr ptr = malloc(sizeof(*ptr)); // allocate space for the new linked list node
    ptr->item = item; // add the new household information to the linked list
    ptr->next = (*head); // the new node will be the new head, ensure the connection to the previous head
...
void printLinkedListByRegion(Node *head, int region) {
    ...
    while(ptr != NULL) {
        ...
        if (ptr->item.region == region)
            printHouseholdItem(++ctr, ptr->item); // only print out the matching household item
        ptr = ptr->next;
    }
    printHouseholdItemSeparator();
    printf("Number of records: %d\n", ctr);
...
/**
 * Display the number of adults tested positive in a specified region with the minimum tested positive as a filter
 *
 * @param head
 * @param region
...
void printLinkedListByRegionWithMinTestedPositive(Node *head, int region, int minTestedPositive) {
    ...
    // iterate through entire linked list
    while (ptr != NULL) {
        // check for matching region with minimum test positives amongst adults (not seniors)
        if (ptr->item.region == region && ptr->item.adultsTestedPositive >= minTestedPositive)
            printHouseholdItem(++ctr, ptr->item); // only print out the matching results
...
void rankPositiveTestedBasedOnTown(Node *head, rank_cases *town) {
    ...
    int bramptonCases = 0, mississaugaCases = 0, mapleCases = 0, vaughanCases = 0, whitbyCases = 0, oshawaCases = 0;
    ...
    // all all town data to a structure for further manipulation (sorting based on cases)
...
void deleteLinkedListItem(NodePtr *head, int region, int town, int race) {
    ...
    while (*head != NULL) {
        ...
        if ((*head)->item.region ==  region && (*head)->item.town == town && (*head)->item.race == race) {
...
void storeDataToFile(Node *head) {
    ...
    printf("Filename: ");
    fgets(fileName, sizeof(fileName), stdin); // get the file name from stdin (user input)
    fileName[strcspn(fileName, "\n")] = 0; // remove the new line character from the filename

    // open the new file with "w" permission. this will ensure that the file will be overwritten if already exists, and if doesn't exist, create it.
    ...
    // add a header to the file corresponding with the data meant to view in that column
    ...
    while (ptr != NULL) {
        ...
        fprintf(file, "%5d\t%7d\t%16d\t%20d\t%20s\t%10s\t%15s\n", ++ctr, ptr->item.size, ptr->item.tested, ptr->item.adultsTestedPositive, displayRace(ptr->item.race), displayRegion(ptr->item.region), displayTown(ptr->item.region, ptr->item.town) );
